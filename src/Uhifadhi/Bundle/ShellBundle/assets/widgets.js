...
/* The wire, mirroring Uhifadhi\Bundle\ShellBundle\Widget\Model\WidgetDom one string at a time. */
...
/* Keep in step with .w-sliding in public/widget.css. */
const SLIDE_MS = 160;

/* As long as a preset name may be — Uhifadhi\Bundle\ShellBundle\Widget\Service\WidgetService::NAME_MAX. */
const NAME_MAX = 60;
...
export function initWidgetLibrary(root = document.querySelector(ROOT_SELECTOR)) {
     ...
     * high its own z-index goes. A shell's shared confirm modal is rendered at
...
/* THE BOOT — the half that did not ship, and the whole of a real bug report:
 * "clicking a preset does not show its preview".
 *
 * Everything above it was correct. The cards carried their kind and their id,
 * the catalogue was embedded, the widgets sat in their templates — and nothing
 * ever called initWidgetLibrary. The module exported it and left the call to
 * the installation, on the same reasoning that leaves the importmap line to the
 * installation; but an importmap entry is a file a bundle MAY NOT WRITE, while
 * a call to its own entry point is simply a line it forgot. No recipe wrote it,
 * the README never asked for it, and so every library page in existence
 * rendered `role="button"` cards with no behaviour behind them.
 *
 * SO THE MODULE ARMS ITSELF, and importing it is the whole contract. This is
 * safe on every page: initWidgetLibrary returns false when there is no library
 * root to arm, and the `armed` WeakSet makes a second call a no-op — a page
 * without a library costs one querySelector. The named export stays for a
 * caller that mounts a library after load.
 *
 * READY OR NOT. A module imported before the body has parsed must wait; one
 * imported after must not wait for an event that has already been and gone. */
