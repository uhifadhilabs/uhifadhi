 ...
 * bundle must not inherit an application's AbstractController), and a trait would have
...
final readonly class WidgetEndpoint
{
    ...
    public function save(Request $request, WidgetCatalog $catalog, ?Uuid $areaUuid = null): Response
    {
        ...
        try {
            ...
            $this->widgets->save($catalog, $user, $payload, $areaUuid);
    ...
    public function applyPreset(Request $request, WidgetCatalog $catalog, string $presetId, ?Uuid $areaUuid = null): Response
    {
        ...
        try {
            $this->widgets->applyPreset($catalog, $user, $areaUuid, $presetId);
    ...
    public function createCustomPreset(Request $request, WidgetCatalog $catalog, ?Uuid $areaUuid = null): Response
    {
        ...
        try {
            ...
            $this->widgets->saveCustomPreset($catalog, $user, $areaUuid, $name, $layout);
        } catch (InvalidWidgetPreferenceException|JsonException $invalid) {
            return new Response($invalid->getMessage(), Response::HTTP_UNPROCESSABLE_ENTITY);
        }
    ...
    public function copyPreset(Request $request, WidgetCatalog $catalog, string $presetId, ?Uuid $areaUuid = null): Response
    {
        ...
        try {
            $this->widgets->copyBuiltinPreset($catalog, $user, $areaUuid, $presetId, $request->request->getString('name'));
        } catch (InvalidWidgetPreferenceException $invalid) {
            return new Response($invalid->getMessage(), Response::HTTP_UNPROCESSABLE_ENTITY);
        }
    ...
    public function applyCustomPreset(Request $request, WidgetCatalog $catalog, Uuid $presetUuid, ?Uuid $areaUuid = null): Response
    {
        return $this->write(
            ...
            $catalog,
            $areaUuid,
            fn (UserInterface $user) => $this->widgets->applyCustomPreset($catalog, $user, $areaUuid, $presetUuid),
        );
    }
    ...
    public function renameCustomPreset(Request $request, WidgetCatalog $catalog, Uuid $presetUuid, ?Uuid $areaUuid = null): Response
    {
        return $this->write(
            ...
            $catalog,
            $areaUuid,
            fn (UserInterface $user) => $this->widgets->renameCustomPreset($catalog, $user, $areaUuid, $presetUuid, $request->request->getString('name')),
        );
    }
    ...
    public function deleteCustomPreset(Request $request, WidgetCatalog $catalog, Uuid $presetUuid, ?Uuid $areaUuid = null): Response
    {
        return $this->write(
            ...
            $catalog,
            $areaUuid,
            fn (UserInterface $user) => $this->widgets->deleteCustomPreset($catalog, $user, $areaUuid, $presetUuid),
        );
    }
    ...
    public function reset(Request $request, WidgetCatalog $catalog, ?Uuid $areaUuid = null): Response
    {
        $user = $this->user();
        $this->denyUnlessCsrfValid($request, $catalog, $areaUuid);
        $this->widgets->reset($catalog, $user, $areaUuid);
     ...
     *
     * @param callable(UserInterface): void $write
     */
    ...
    /**
     * A layout belongs to a person; without one there is nothing to read or
     * write.
     *
     * TWO USER INTERFACES MEET HERE, and this is the one place the difference is
     * visible. The token storage answers with Symfony's — "who is signed in" —
     * while a widget preference is a record ABOUT somebody, which is what
     * {@see UserInterface} asks. An installation's account class satisfies both,
     * so the check is simply whether the signed-in principal is one: a firewall
     * that authenticated something else (an API client, a stateless token) has
     * nobody to keep a dashboard for, and this says so rather than inventing a
     * person.
     */
