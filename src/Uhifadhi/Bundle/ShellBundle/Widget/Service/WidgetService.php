...
use Uhifadhi\Bundle\ShellBundle\Widget\Exception\InvalidWidgetPreferenceException;
use Uhifadhi\Bundle\ShellBundle\Widget\Exception\UnknownWidgetPresetException;
...
/**
 * THE WIDGET FRAMEWORK: one person's layout of one dashboard surface, resolved
 * from that surface's {@see WidgetCatalog} and their stored row.
 *
 * The catalogue is handed in rather than known here, which is the whole design:
 * every dashboard in an installation arranges itself by the same merge,
 * validation and clamping rules, and a module ships a catalogue rather than a
...
final class WidgetService
{
     ...
     */
    public function resolve(WidgetCatalog $catalog, ?UserInterface $user, ?Uuid $areaUuid = null): array
    {
        if (null === $user) {
            return self::merge($catalog, null);
        }

        $row = $this->preferences->findOneForUser($catalog->surface, $user, $areaUuid);
     ...
     */
    public function activeRef(WidgetCatalog $catalog, ?UserInterface $user, ?Uuid $areaUuid = null): array
    {
        $row = null !== $user
            ? $this->preferences->findOneForUser($catalog->surface, $user, $areaUuid)
            : null;
     ...
     */
    public function save(WidgetCatalog $catalog, UserInterface $user, array $payload, ?Uuid $areaUuid = null): void
    {
        $row = $this->row($catalog, $user, $areaUuid);
     ...
     */
    public function applyPreset(WidgetCatalog $catalog, UserInterface $user, ?Uuid $areaUuid, string $presetId): void
    {
        $preset = self::presetOf($catalog, $presetId);

        $this->store($catalog, $user, $areaUuid, $preset, WidgetPreference::KIND_DESIGN, $preset->id);
    }
     ...
     */
    public function copyBuiltinPreset(WidgetCatalog $catalog, UserInterface $user, ?Uuid $areaUuid, string $presetId, ?string $rawName = null): WidgetCustomPreset
    {
        $source = self::presetOf($catalog, $presetId);
        $name = null !== $rawName && '' !== trim($rawName)
            ? self::presetName($rawName)
            : $this->freeName($catalog, $user, $areaUuid, $source->label.' — copy');
        ...
        $row = new WidgetCustomPreset($catalog->surface, $user, $areaUuid, $name, $layout);
        $this->entityManager->persist($row);
        $this->entityManager->flush();

        $this->applyCustomPreset($catalog, $user, $areaUuid, self::uuidOf($row));
     ...
     */
    public function customPresets(WidgetCatalog $catalog, ?UserInterface $user, ?Uuid $areaUuid = null): array
    {
        if (null === $user) {
            return [];
        }
        ...
        return array_map(
            ...
            $this->savedPresets->findForUser($catalog->surface, $user, $areaUuid),
        );
    }
     ...
     */
    public function saveCustomPreset(WidgetCatalog $catalog, UserInterface $user, ?Uuid $areaUuid, string $rawName, ?array $layout = null): WidgetCustomPreset
    {
        $name = self::presetName($rawName);
        $captured = null !== $layout
            ? self::composedLayout($catalog, $layout)
            : self::captureLayout($this->resolve($catalog, $user, $areaUuid));
        // Refuses an empty dashboard, and refuses it BEFORE any row is touched.
        self::customPreset('draft', $name, $captured);

        $row = $this->savedPresets->findOneNamed($catalog->surface, $user, $areaUuid, $name)
            ?? new WidgetCustomPreset($catalog->surface, $user, $areaUuid, $name);
     ...
     */
    public function applyCustomPreset(WidgetCatalog $catalog, UserInterface $user, ?Uuid $areaUuid, Uuid $uuid): void
    {
        $row = $this->ownedPreset($catalog, $user, $areaUuid, $uuid);
        $preset = self::customPreset((string) $row->getUuidString(), $row->getName(), $row->getLayout());

        $this->store($catalog, $user, $areaUuid, $preset, WidgetPreference::KIND_MINE, $preset->id);
    }
     ...
     * @throws UnknownWidgetPresetException     when the preset is not theirs, or gone
     */
    public function renameCustomPreset(WidgetCatalog $catalog, UserInterface $user, ?Uuid $areaUuid, Uuid $uuid, string $rawName): void
    {
        $row = $this->ownedPreset($catalog, $user, $areaUuid, $uuid);
        $name = self::presetName($rawName);

        $clash = $this->savedPresets->findOneNamed($catalog->surface, $user, $areaUuid, $name);
     ...
     * @throws UnknownWidgetPresetException when the preset is not theirs, or gone
     */
    public function deleteCustomPreset(WidgetCatalog $catalog, UserInterface $user, ?Uuid $areaUuid, Uuid $uuid): void
    {
        $preset = $this->ownedPreset($catalog, $user, $areaUuid, $uuid);
        $wasActive = $uuid->toRfc4122() === $this->activeRef($catalog, $user, $areaUuid)['id'];

        $this->entityManager->remove($preset);
        $this->entityManager->flush();

        if ($wasActive) {
            $this->reset($catalog, $user, $areaUuid);
        }
    }

    /** Back to the catalogue's layout — no row means the defaults, so reset deletes. */
    public function reset(WidgetCatalog $catalog, UserInterface $user, ?Uuid $areaUuid = null): void
    {
        $row = $this->preferences->findOneForUser($catalog->surface, $user, $areaUuid);
        if (null === $row) {
            return;
     ...
     */
    private function store(WidgetCatalog $catalog, UserInterface $user, ?Uuid $areaUuid, WidgetPreset $preset, string $kind, string $presetId): void
    {
        $row = $this->row($catalog, $user, $areaUuid);
    ...
    private function row(WidgetCatalog $catalog, UserInterface $user, ?Uuid $areaUuid): WidgetPreference
    {
        return $this->preferences->findOneForUser($catalog->surface, $user, $areaUuid)
            ?? new WidgetPreference($catalog->surface, $user, $areaUuid);
    }
     ...
     */
    private function freeName(WidgetCatalog $catalog, UserInterface $user, ?Uuid $areaUuid, string $wanted): string
    {
        $base = mb_substr(trim($wanted), 0, self::NAME_MAX);
        $name = $base;
        for ($n = 2; null !== $this->savedPresets->findOneNamed($catalog->surface, $user, $areaUuid, $name); ++$n) {
            $suffix = ' '.$n;
     ...
     */
    private function ownedPreset(WidgetCatalog $catalog, UserInterface $user, ?Uuid $areaUuid, Uuid $uuid): WidgetCustomPreset
    {
        return $this->savedPresets->findOwned($catalog->surface, $user, $areaUuid, $uuid)
