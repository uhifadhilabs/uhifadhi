 ...
 *    {@see AreaInterface} (published by the contracts) and never at a class;
 *    the registry's per-area row is the oldest of them. This entity is the answer,
 ...
 *    a naming preference. Before this bundle existed, every installation was
 ...
 *    rename migration when it adopts this bundle — it deletes its class,
...
{
    public function testItAnswersThePlatformsAreaContract(): void
    {
        ...
        self::assertTrue(
            ...
            'the registry maps its per-area row at AreaInterface; this entity is what answers it',
        );
    }
    ...
    /**
     * IDENTITY IS THE CONTRACT'S FLOOR: getId() is always part of the area contract.
     *
     * The contract exposes more — getName() and getUuidString(), so a consumer
     * holding an area only as AreaInterface can name and address it across a
     * module boundary. Pinning the EXACT surface is the contracts' own job; this
     * consumer test asserts only what this bundle depends on — that identity is
     * there — and stays green when the shared contract legitimately grows.
     */
